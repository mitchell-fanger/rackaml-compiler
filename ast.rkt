...
(struct Lam    (n xs e)     #:prefab)    ;; xs = (x y z LST) LamVar(n xs list-arg e)
(struct Var    (x)          #:prefab)
(struct App    (f es)       #:prefab)

;; FINAL PROJECT: apply can take a function and a list of arguments
;; lst is NOT a list itself. it is an expression that evaluates to a list of arbitrary size
(struct Apply (f lst)       #:prefab)
...
(define (fvs e)
  (define (fvs e)
    (match e
      ...
      ;; FINAL PROJECT: need to get free vars of it too
      [(Apply f lst)   (append (fvs f) (fvs lst))]
...
(define (desugar e+)
  (match e+
    ...
    ;; FINAL PROJECT: need to desugar the apply's as well
    [(Apply f lst)   (Apply (desugar f) (desugar lst))]
...
;    [(Lam n xs e)    (Lam n xs (label-λ e))]    
    ...
    ;; FINAL PROJECT: added case for Apply, which can have nested structures
    ;; doesn't have to be mapped on lst, because we expect lst to be something that evaluates to a cons
    [(Apply f lst)   (Apply (label-λ f) (label-λ lst))]
...
(define (λs e)
  (match e
    ...
    ;; FINAL PROJECT: 
    [(Apply f lst)   (append (λs f) (λs lst))]
